/*
	...
	CalcFuzzyHash function in this program utilizes fuzzy_hash_buf, one of the
	APIs from ssdeep project.
	- FUZZY HASHING API
...
int xwf_version = 0;
const wchar_t* XT_VER = L"XT_FUZZY - v1.0.0";
...
int CalcFuzzyHash(BYTE* filebuf, DWORD fsize, wchar_t* fuzzy) {

	char buf[149] = { 0x00 };
	int ret = 0;
	size_t r = 0;
	errno_t err = 0;
	ret = fuzzy_hash_buf(filebuf, sizeof(unsigned char) * fsize, buf);
	if (ret != 0) {
		return ret;
	}
	err = mbstowcs_s(&r, fuzzy, 150, buf, _TRUNCATE);
	return err;
...
LONG __stdcall XT_Init(CallerInfo info, DWORD nFlags, HANDLE hMainWnd, void* lpReserved){
	...
	if (info.version < MIN_VER){
		wcscpy_s(msg, L"XT_FUZZY: The Version of X-Ways Forensics must be v.");
	...
	if (XWF_GetCaseProp(NULL, XWF_CASEPROP_TITLE, case_name, NAME_BUF_LEN) == -1){
		XWF_OutputMessage(L"XT_FUZZY: Active Case is Required. Exiting...", 0);
		EXIT = TRUE;
		return 1;
	}

	if (!XWF_GetFirstEvObj(NULL)){
		XWF_OutputMessage(L"XT_FUZZY: No Evidence is Found. Exiting...", 0);
		EXIT = TRUE;
		return 1;
	}
...
LONG __stdcall XT_Prepare(HANDLE hVolume, HANDLE hEvidence, DWORD nOpType, void* lpReserved){
	...
	switch (nOpType){
		case XT_ACTION_RUN:
			XWF_OutputMessage(L"XT_FUZZY: Not Supposed to be Executed from the Tools Menu. Exiting...",0);
			return -1;
		case XT_ACTION_LSS:
		case XT_ACTION_PSS:
		case XT_ACTION_SHC:
			XWF_OutputMessage(L"XT_FUZZY: Not Supposed to be Executed during Searches. Exiting...", 0);
			return -3;
		case XT_ACTION_RVS:
		...
		case XT_ACTION_DBC:
			...
			break;
		default:
			XWF_OutputMessage(L"XT_FUZZY: Does Not Support this Mode of Operation. Exiting...", 0);
			return -1;
	}
...
LONG __stdcall XT_Finalize(HANDLE hVolume, HANDLE hEvidence, DWORD nOpType, void* lpReserved){
	...
	XWF_ShowProgress((wchar_t*)TEXT("Calculating Fuzzy Hash..."), 4);
	...
	for (INT64 i = 0; i < fc; i++) {
		...
		if (hItem != 0) {
			...
			if (filebuf == NULL){
				XWF_Close(hItem);
				wcscpy_s(msg, L"XT_FUZZY: Unable to Allocate Memory for \"");
				wcscat_s(msg, item);
			...
			XWF_Close(hItem);
			wchar_t fuzzy[150];
			int ret = CalcFuzzyHash((BYTE*)filebuf, actual_size, fuzzy);
			if (ret == 0) {
				XWF_AddComment(file_ids[i].xwf_id, (LPWSTR)fuzzy, 0);
			} else {
				wcscpy_s(msg, L"XT_FUZZY: Unable to Calculate Fuzzy Hash of \"");
				wcscat_s(msg, item);
				wcscat_s(msg, L"\". Skipping...");
				XWF_OutputMessage(msg, 0);
			}
...
LONG __stdcall XT_ProcessItem(LONG nItemID, void* lpReserved) {
	...
	if (current_volume == NULL) {
		XWF_OutputMessage(L"XT_FUZZY: Unable to Associate the File with a Volume. Exiting...", 0);
		return -1;
	}
